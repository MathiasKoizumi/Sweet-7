...
namespace WindowsFormsApplication1
{
	internal class Jingo
	{
		...
		public Jingo changeCourse()
		{
			...
			{
				int num = new Random().Next(waypoints.Length);
				x += (int)((double)(waypoints[num].X + 300) * Math.Atan2(244.0, 240.0));
				y += (int)((double)(waypoints[num].Y + 550) * Math.Exp(new Random().Next(220)));
			}
			pointy.X = x;
			pointy.Y = y;
			return this;
		...
		public Point[] getWaypoints()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				array[i] = new Point(new Random().Next(2000), new Random().Next(2000));
			}
			return array;
		}
